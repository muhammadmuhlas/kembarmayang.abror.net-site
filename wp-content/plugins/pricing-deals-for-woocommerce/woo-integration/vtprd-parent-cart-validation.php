...
/* //v1.1.1 TEST test test removed  
    //v1.0.7.2  needed if all prices are zero from Catalog rules, otherwise subtotal reflects list price!
    add_action( 'woocommerce_before_mini_cart', array(&$this, 'vtprd_maybe_recalc_woo_totals'), 5, 1 );  //v1.1.0.6 changed priority from 10 to 5
*/
          ...
          //this will now pick up BOTH inline discounts, and solo CATLOG discounts...
              ...
              switch( true ) {
                ...
                case ($vtprd_cart_item->product_discount_price_woo > 0)  :               
                    $cart_item_value['data']->price = $this->vtprd_choose_mini_cart_price($vtprd_cart_item);   //$vtprd_cart_item->discount_price;    //
                    $mini_cart_updated = true;                    
                  break; 
    ...
    */
    ...
    foreach ( $cart_object->cart_contents as $cart_item_key => $cart_item_value ) {
      ...
      foreach($vtprd_cart->cart_items as $vtprd_key => $vtprd_cart_item) { 
        ...
        //this will now pick up BOTH inline discounts, and solo CATLOG discounts...
           ...
           if ($vtprd_setup_options['discount_taken_where'] == 'discountUnitPrice')  {
             ...
             if ( ($vtprd_cart_item->product_discount_price_woo > 0) ||
                  ($vtprd_cart_item->unit_price < $cart_item_value['data']->price) ) { //v1.0.9.3 pick up CATALOG-only discount when discountCoupon!!           
                $cart_item_value['data']->price = $vtprd_cart_item->unit_price;   //$vtprd_cart_item->discount_price;    //
             }
       ...
       //error_log( print_r(  'vtprd_maybe_before_calculate_totals $cart_object', true ) );
      ...
      if ( !is_admin() ) {
          ...
          if ( (class_exists('WC_Product_Addons')) &&
               (get_option('vtprd_ruleset_has_a_display_rule') == 'yes') ) {    
                add_filter('woocommerce_get_price',                   array(&$this, 'vtprd_maybe_get_price'), 10, 2);  //only way to get the price to the faux variations... 
                $filter_added = true;            
          }
          ...
          if ( ($do_get_price) &&
               (!$filter_added) ) {    
                add_filter('woocommerce_get_price',                   array(&$this, 'vtprd_maybe_get_price'), 10, 2);  //allows single_product call to use get_price
                //NOT NEEDED??????????????????
